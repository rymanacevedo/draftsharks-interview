...
createApp({
  ...
  computed: {
    ...
    availablePositions() {
      const positions = this.selectedTeamPositions;
      return Object.fromEntries(
        Object.entries(positions).filter(([_, players]) => players.length > 0),
      );
    },
  },
  methods: {
  ...
  template: `
    ...
    <card class="mt3 mb3" :heading="selectedTeamName + ' ' + selectedCategory">
      ...
      <player-table :players="selectedPlayers" :sortOrder="sortPlayersOrder[selectedCategory]"></player-table>
    </card>
  `,
}).mount('#app');
